...
class Trace {
    ...
    class Process: Codable {
        ...
        func normalize(base: FilePath, path: FilePath) -> FilePath {
            ...
            guard let resolved = try? FileManager.default.destinationOfSymbolicLink(atPath: fullPath.string) else {
                return fullPath
            }
    ...
    init(root: pid_t, selfPid: pid_t = getpid()) {
        ...
        let rootProc = Process(pid: root, parent: 0, uid: 0, image: find(path: "/__root__"), cwd: FilePath(FileManager.default.currentDirectoryPath))
        procs[root] = rootProc

        let selfProc = Process(pid: selfPid, parent: 0, uid: 0, image: find(path: "/__self__"), cwd: FilePath(FileManager.default.currentDirectoryPath))
    ...
    func withProcess(_ event: UnsafeMutablePointer<mkcheck2_event_header>, _ body: (inout Process) throws -> Void) rethrows {
    ...
    func withEvent(_ eventHeader: UnsafeMutablePointer<mkcheck2_event_header>, _ body: (UnsafeMutablePointer<mkcheck2_event>) throws -> Void) throws {
        try eventHeader.withMemoryRebound(to: mkcheck2_event.self, capacity: 1) {
            #if DEBUG
            let path = (try? $0.pathOrInode) ?? "bad string"
            logger.trace("\(eventHeader.pointee.type) \(eventHeader.pointee.pid) \(eventHeader.pointee.uid) LINE=\(eventHeader.pointee.source_line) \(path)")
            #endif
            try body($0)
        }
    }

    func withFatEvent(_ eventHeader: UnsafeMutablePointer<mkcheck2_event_header>, _ body: (UnsafeMutablePointer<mkcheck2_fat_event>) throws -> Void) rethrows {
        try eventHeader.withMemoryRebound(to: mkcheck2_fat_event.self, capacity: 1) {
            ...
            logger.trace("\(eventHeader.pointee.type) \(eventHeader.pointee.pid) \(eventHeader.pointee.uid) LINE=\(eventHeader.pointee.source_line) \(path1) \(path2)")
            #endif
            try body($0)
        }
    }

    func withFat2Event(_ eventHeader: UnsafeMutablePointer<mkcheck2_event_header>, _ body: (UnsafeMutablePointer<mkcheck2_fat2_event>) throws -> Void) rethrows {
    ...
    func handleEvent(_ eventHeader: UnsafeMutablePointer<mkcheck2_event_header>) throws {
        ...
        case .eventTypeRename:
            try withFatEvent(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (source?, dest?) = try event.paths else { fatalError() }
                    process.rename(source: process.normalize(path: source), dest: process.normalize(path: dest), trace: self)
                }
            }
        case .eventTypeRenameAt:
            try withFat2Event(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (sourceBasePath?, destBasePath?, source?, dest?) = try event.paths else { fatalError() }
        ...
        case .eventTypeLink:
            try withFatEvent(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (source?, dest?) = try event.paths else { fatalError() }
                    process.link(target: process.normalize(path: source), linkPath: process.normalize(path: dest), trace: self)
                }
            }
        case .eventTypeLinkAt:
            try withFat2Event(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (sourceBasePath?, destBasePath?, sourceLink?, destLink?) = try event.paths else { fatalError() }
                    let sourceBase = process.normalize(path: sourceBasePath)
                    let destBase = process.normalize(path: destBasePath)
                    process.link(
        ...
        case .eventTypeSymlink:
            try withFatEvent(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (sourceLink?, destRelative?) = try event.paths else { fatalError() }
        ...
        case .eventTypeSymlinkAt:
            try withFat2Event(eventHeader) { event in
                try withProcess(eventHeader) { process in
                    guard case let (base?, sourceLink?, destRelative?, _) = try event.paths else { fatalError() }
