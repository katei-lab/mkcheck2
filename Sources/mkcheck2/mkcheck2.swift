...
struct Mkcheck2: ParsableCommand {
    ...
    struct Diff: ParsableCommand {
        ...
        private func shouldSkip(_ info: Serialization.FileInfo) -> Bool {
            let path = info.name
            return path.starts(with: "/tmp/") || path.starts(with: "/proc/") || path.starts(with: "/dev/") || info.deleted == true
        }
    ...
    static func dropPrivileges() throws {
        ...
        guard let sudoGID = ProcessInfo.processInfo.environment["SUDO_GID"],
              let sudoUID = ProcessInfo.processInfo.environment["SUDO_UID"] else {
...
extension UnsafeMutablePointer where Pointee == mkcheck2_event {
    var pathString: String? {
        get throws {
            return try UnsafeRawPointer(self).advanced(by: MemoryLayout<mkcheck2_event>.offset(of: \.path)!).withMemoryRebound(to: mkcheck2_path_t.self, capacity: 1) { base -> String? in
...
extension UnsafeMutablePointer where Pointee == mkcheck2_fat_event {
    var pathStrings: (String?, String?) {
        get throws {
            let path0 = try UnsafeRawPointer(self).advanced(by: MemoryLayout<mkcheck2_fat_event>.offset(of: \.path.0)!).withMemoryRebound(to: mkcheck2_path_t.self, capacity: 1) { base -> String? in
                return try base.readPathString()
            }
            let path1 = try UnsafeRawPointer(self).advanced(by: MemoryLayout<mkcheck2_fat_event>.offset(of: \.path.1)!).withMemoryRebound(to: mkcheck2_path_t.self, capacity: 1) { base -> String? in
                return try base.readPathString()
            }
            return (path0, path1)
        }
...
extension UnsafeMutablePointer where Pointee == mkcheck2_fat2_event {
    var pathStrings: (String?, String?, String?, String?) {
        get throws {
            func readPathString(at keyPath: KeyPath<mkcheck2_fat2_event, mkcheck2_path_t>) throws -> String? {
                let offset = MemoryLayout<mkcheck2_fat2_event>.offset(of: keyPath)!
                return try UnsafeRawPointer(self).advanced(by: offset).withMemoryRebound(to: mkcheck2_path_t.self, capacity: 1) { base -> String? in
                    return try base.readPathString()
                }
            }
            return try (
    ...
    var paths: (FilePath?, FilePath?, FilePath?, FilePath?) {
        get throws {
            ...
            return (path0.map { FilePath($0) }, path1.map { FilePath($0) }, path2.map { FilePath($0) }, path3.map { FilePath($0) })
        }
    }
}
...
extension UnsafePointer where Pointee == mkcheck2_path_t {
    func readPathString() throws -> String? {
        ...
        return try self.withMemoryRebound(to: CChar.self, capacity: Int(DEFAULT_SUB_BUF_LEN * DEFAULT_SUB_BUF_SIZE)) { base -> String? in
            var buffer: [String] = []
