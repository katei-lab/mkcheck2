...
struct TestUtils: ParsableCommand {
    ...
    struct Write: ParsableCommand {
        ...
        func run() throws {
            ...
            let fd = try FileDescriptor.open(path, .writeOnly, options: options, permissions: .ownerReadWriteExecute)
    ...
    struct ForkExecveAt: ParsableCommand {
        ...
        func run() throws {
            ...
            try programPath.withCString { programPath in
                ...
                let argv: [UnsafeMutablePointer<CChar>?] = [programPath] + arguments.map { strdup($0) } + [nil]
